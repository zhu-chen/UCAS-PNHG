...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(os.path.dirname(os.path.abspath(__file__)), "bart_title_generator.log")),
...
class PersonalizedTitleGenerator:
    def __init__(self, model_name="facebook/bart-base", max_length=384, load_from_local=False, local_model_path=None):
        """初始化个性化标题生成器，支持从本地加载模型"""
        ...
        # 加载预训练模型和分词器
        if self.load_from_local and self.local_model_path:
            self.local_model_path = os.path.abspath(self.local_model_path)
            logger.info(f"从本地路径 {self.local_model_path} 加载模型和分词器")
            try:
    ...
    def train(self, batch_size=1, num_epochs=3, learning_rate=5e-5):
        ...
        if len(self.train_dataset) > 0:
            ...
            monitor_memory()
            logger.info(f"模型保存至 {best_model_dir}")
...
if __name__ == "__main__":
    ...
    # 从本地加载模型示例（请替换为实际路径）
    local_model_path = "/root/newstitle/UCAS-PNHG/scripts/local_bart_model"
